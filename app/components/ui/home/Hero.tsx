        ...
        {t.rich("title", {
          screen: (chunks) => (
            <span className="relative inline-flex items-center">
              ...
              <img src="/svg/mockups.svg" alt="" aria-hidden="true" className="inline-block h-[1.6em] w-auto align-middle translate-y-[0.1em] sm:translate-y-[0.3em]" />
              <img src="/svg/cursor-animate.svg" className="hidden sm:flex absolute -top-18 sm:-top-25 -right-28 sm:-right-30 h-[4em] w-auto" alt="" aria-hidden="true" />
            </span>
          ),
        })}
        <br />
        ...
        <div className="flex flex-col items-center gap-2 w-full sm:w-72">
          ...
          >
            <div className="flex items-center justify-center gap-3 pointer-events-none w-full">
              {isUploadingVideo ? (
                <>
                  <Icon icon="svg-spinners:ring-resize" width="18" className="text-blue-400 shrink-0" aria-hidden="true" />
                  <span>{t("uploadButtonUploading")}</span>
                </>
              ) : isDraggingVideo ? (
                <>
                  <Icon icon="ph:arrow-fat-down-bold" width="18" className="text-blue-400 shrink-0" aria-hidden="true" />
                ...
                <>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white/70 text-xs font-mono select-none">MP4, WebM, MOV</span>
                  </div>
                </>
              )}
            </div>
        ...
        <div className="flex flex-col items-center gap-2 w-full sm:w-72">
          ...
          >
            <div className="flex items-center justify-center gap-3 pointer-events-none w-full">
              {isUploadingPhoto ? (
                <>
                  <Icon icon="svg-spinners:ring-resize" width="18" className="text-red-400 shrink-0" aria-hidden="true" />
                  <span>{t("uploadPhotoUploading")}</span>
                </>
              ) : isDraggingPhoto ? (
                <>
                  <Icon icon="ph:arrow-fat-down-bold" width="18" className="text-red-400 shrink-0" aria-hidden="true" />
                  <span>{t("uploadPhotoDragging")}</span>
                </>
              ) : (
                <>
                  <Icon icon="solar:gallery-wide-linear" width="20" className="shrink-0" aria-hidden="true" />
                  <div className="flex items-center gap-2">
                    <span>{t("uploadPhotoButton")}</span>
                    <span className="text-white/70 text-xs font-mono select-none">JPG, PNG, WEBP</span>
                  </div>
                </>
              )}
            </div>
